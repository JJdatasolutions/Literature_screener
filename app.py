...
def extract_all_pages(file_buffer):
    """Slaat het boek pagina per pagina op, zodat we razendsnel kunnen filteren via de UI."""
...
if uploaded_file is not None:
    ...
    st.sidebar.info("To prevent memory crashes on large books (like *The Shining*), limit the scope of the analysis.")
    ...
    # Combine text based on user selection
    ...
    with tab4:
        ...
        if st.button("Run Gender Analysis"):
            ...
            stop_v = {'be', 'have', 'do', 'go', 'get', 'know', 'think', 'say', 'see', 'look', 'come', 'tell', 'ask', 'seem', 'turn', 'move', 'start', 'begin', 'stop', 'use', 'try', 'feel', 'leave', 'make', 'take', 'give', 'find', 'call', 'want', 'let', 'put', 'keep', 'show', 'hold', 'bring', 'become', 'mean'}
    ...
    with tab5:
        ...
        if st.button("Extract Colors"):
            ...
            for w in words:
                if w in colors: found_colors[w] += 1
            gc.collect()
            ...
            if found_colors:
                ...
                for c_name, count in found_colors.most_common():
                    ...
                    if width > 0.05:
                        ax.text(left + width/2, 0, f"{c_name}\n{int(width*100)}%", ha='center', va='center', color='white' if c_name not in ['white', 'yellow', 'gold', 'f0f0f0'] else 'black', fontweight='bold', fontsize=10)
                    left += width
                ax.set_yticks([])
                ax.set_xticks([])
    ...
    # ==========================================
    # TAB 6: DYNAMIC RELATIONSHIP TRACKER (NEW)
    # ==========================================
    with tab6:
        st.header("📈 Relationship Evolution Tracker")
        st.info("Track the emotional dynamic between two specific characters over the course of the selected pages. We analyze sentiment whenever both names appear in the same sentence.")
        
        col1, col2 = st.columns(2)
        char1 = col1.text_input("First Character (e.g., Jack)", "Jack")
        char2 = col2.text_input("Second Character (e.g., Wendy)", "Wendy")

        if st.button("Trace Relationship"):
            if not char1 or not char2:
                st.warning("Please enter two character names.")
            else:
                with st.spinner(f"Scanning the text for interactions between {char1} and {char2}..."):
                    sentences = re.split(r'(?<=[.!?]) +', text_data)
                    progression = []
                    sentiments = []

                    for i, s in enumerate(sentences):
                        # Controleer of BEIDE namen in de zin staan (niet hoofdlettergevoelig)
                        if re.search(rf'\b{re.escape(char1)}\b', s, re.IGNORECASE) and re.search(rf'\b{re.escape(char2)}\b', s, re.IGNORECASE):
                    ...
                    if len(sentiments) < 3:
                        st.warning(f"Not enough direct interactions found between **{char1}** and **{char2}** in these {selected_page_count} pages to draw a trendline. Try expanding the page limit or selecting different characters.")
                    else:
                        df = pd.DataFrame({'Progress': progression, 'Sentiment': sentiments})
                        
                        fig, ax = plt.subplots(figsize=(10, 5))
                        # Punten plotten (de daadwerkelijke zinnen)
                        sns.scatterplot(data=df, x='Progress', y='Sentiment', color='#1f77b4', s=60, alpha=0.6, ax=ax, label="Direct Interaction")
                        
                        # Polynomial trendline (Orde 3 geeft een mooie curve)
                        sns.regplot(data=df, x='Progress', y='Sentiment', scatter=False, order=3, color='#e41a1c', label="Emotional Trend")
